...
const dinoGrammar = ohm.grammar(fs.readFileSync("src/rawrdino.ohm"))
...
function mustNotAlreadyBeDeclared(context, name, at) { //clarify if this..
...
export default function analyze(sourceCode) {
  ...
  const analyzer = dinoGrammar.createSemantics().addOperation("rep", {
    ...
    Statement_vardec(modifier, id, _eq, initializer) { //VARIABLE DECLARATION
    ...
    Statement_fundec(_fun, id, _open, params, _close, body) { //FUNCTION DECLARATION: not sure if completely works atm. I don't think allows for recurs
    ...
    Statement_assign(id, _eq, expression) { //assign if not read only.
      const source = expression.rep()
      const target = id.rep()
      mustNotBeReadOnly(target)
      return new core.Assignment(target, source)
    },
    Statement_print(_print, argument) { //simple print
      return new core.PrintStatement(argument.rep())
    },
    Statement_return(_return, argument) { //RETURN: remember our return is "hatch", check that must be in a function.
    ...
    Statement_break(_break) { //readOnly line is not working for break.
      mustBeInLoop(context)
      return new core.BreakStatement()
    },
    IfStmt_long(_if, test, consequent, _else, alternate) { //IF, ELSE IF
    ...
    IfStmt_short(_if, test, consequent) { //short if statement, similar code. May need to touch up these ifs.
        const testRep = test.rep()
        mustHaveBooleanType(testRep, test)
        const consequentRep = consequent.rep()
        return new core.ShortIfStatement(testRep, consequentRep)
    },
    IfStmt_elsif(_if, test, consequent, _else, alternate) { //no new context.
      const testRep = test.rep()
      mustHaveBooleanType(testRep)
      const consequentRep = consequent.rep()
      const alternateRep = alternate.rep()
    ...
    LoopStmt_while(_while, test, body) { //WHILE, similar to carlos but different.
      const test = test.rep()
      const body = body.rep()
    ...
    */
    LoopStmt_range(_for, id, _in, low, op, high, body) {// from carlos.
